...
\begin{itemize}
\invisible<1-2>{\item[-] $F = \{\text{Myanmar sells oil to China} \} $\\ } \pause 
\invisible<1-3>{\item[-] $E = \{\text{Myanmar discovers oil} \} $ \\ }  \pause 
\invisible<1-4>{\item[-] If $F$ occurs then $E$ must occur, $P(E|F) = 1$ \\ } \pause 
\end{itemize}


\invisible<1-5>{Example 2: Security Council voting on Syria strike} \pause 
\begin{itemize}
\invisible<1-6>{\item[-] $V = \{ \text{Russia and China veto strike}\} $}\pause
\invisible<1-7>{\item[-] $A = \{ \text{Strike approved by UN}\}$}  \pause 
\invisible<1-8>{\item[-] $P(A|V)  = \frac{P(A \cap V)}{P(V) } = \frac{P(\emptyset)}{P(V)}$ = 0}
\end{itemize}
}
...
\invisible<1-5>{$\Omega = \{(\text{fair}, H), (\text{fair}, T), (\text{bias}, H), (\text{bias}, T) \}$} \pause 
...
\invisible<1>{Suppose you're on a game show, and you're given the choice of three doors: Behind one door is a car; behind the others, goats. \\} \pause 
\begin{itemize}
\invisible<1-2>{\item[-] A contestant guesses a door.} \pause 
\invisible<1-3>{\item[-] The host opens a different door, revealing one of two goats. The contestant has option to switch} \pause 
...
\invisible<1-7>{If $C$ is revealed to not have a car:\\} 
...
\invisible<1-5>{\alert{You can double the chances of winning with switch}}
