...
\begin{itemize}
\item $X(1)=3^{1-1}$\pause$=3^0=1$\pause
\item $X(2)=3^{2-1} = 3$
\item $X(3)=3^{3-1} = 9$
\item $X(4)=3^{4-1} = 27$
\item $X(5)=3^{5-1} = 81$
...
Let $X(\omega)$ be the number of dots to the 0 power -1.\pause

\begin{itemize}
\item $X(1)=1^0-1= 0$\pause
\item $X(2)=2^0-1 = 0$
\item $X(3)=3^0-1 = 0$
\item $X(4)=4^0-1 = 0$
\item $X(5)=5^0-1 =0$
...
\item $P(X=x)\pause $\invisible<1-4>{$ \equiv P(\{\omega \in \Omega : X(\omega)= x\})$}
...
\invisible<1>{Value of random variable for any outcome weighted by the probability of observing that outcome} \pause \\
\invisible<1-2>{probability weighted average} \pause \\
...
\tiny{
\begin{eqnarray} 
\invisible<1-6>{E[(X - E[X])^2] & = & \sum_{x} (x  - E[X])^2p(x) \nonumber \\ } \pause 
\invisible<1-7>{& = & \sum_{x} \left(x^2  -E[X] x -x E[X] + E[X]^2\right) p(x) \nonumber \\ } \pause 
\invisible<1-8>{& = & \sum_{x} x^2 p(x)   - \sum_{x} 2x E[X]p(x) + \sum_{x} E[X]^2 p(x) \nonumber \\ } \pause 
\invisible<1-9>{& = & \sum_{x} x^2 p(x)   - 2E[X] \sum_{x} x p(x) + \sum_{x} E[X]^2 p(x) \nonumber \\ } \pause 
\invisible<1-10>{&  = &  E[X^2] - 2E[X]^2 + E[X]^2  \nonumber \\ } \pause 
\invisible<1-11>{& = & E[X^2] - E[X]^2 \nonumber \\ } \pause 
\invisible<1-12>{& = & \text{Var}(X) \nonumber } 
\end{eqnarray}
}
\end{itemize}
