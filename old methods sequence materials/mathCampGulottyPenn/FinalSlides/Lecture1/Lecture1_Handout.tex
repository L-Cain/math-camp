...
\item \textcolor{myblack}{Course context}
...
\item Followup: PPHA 44200, PLSC 30700, Game Theory I
\end{itemize}
...
\end{itemize}
\item Social Choice Theory (M. Nalepa) (PLSC 408001)
\begin{itemize}
\item Normative and positive models of decision-making.
\item Uses: understanding societies, micro-foundations for game theory, 
\end{itemize}
\end{itemize}
\end{frame}
...
\item Do not skip steps.  If you lack high school math, get high school math (not this course).\pause
...
\item If an idea is too complex, find a simpler presentation.
...
\item This course is the broadest offered in the department: we cover 300+ years of mathematical insights. \pause Don't expect your understanding to be immediate, but demand clarification in class.\pause
\item ``The Owl of Minerva begins its flight at the break of dawn.'' -Hegel \pause
\item Mathematical training has advantages, hundreds of thousands have gone before you.  
...
\item The textbook\textbf{s} are great, go back to them often.
\end{itemize}\pause
...
\item Assignments can be worked on in groups, but all answers must be your own.
\end{itemize}
\item Practice, practice, practice
\begin{itemize}
\item We will be offering practice problems every week related to the course.
\item Our TA, Arthur, will further provide worked examples.
\item My office hours are available as well, from 11:00-12:00 on Tuesdays and Thursdays.
\end{itemize}
\end{itemize}
...
{Examples: }
\begin{itemize}
\item $\{$Arthur, Bobby$\} \cup \{2, 5, 3, 8, 10\}$ \\ =$\{$Arthur, Bobby$, 2, 5, 3, 8, 10\}$ \pause
\item $\{$Arthur, Bobby$\} \cup \{2, 5, 3, 8, 10\} \cup \{$red, green, blue$\}$ \\ $=\{$Arthur, Bobby$, 2, 5, 3, 8, 10$, red, green, blue$\}$ \pause
\item $\{$Arthur, Bobby$\} \cup \{$Arthur, Bobby$\} =\{$Arthur, Bobby$\}$
\end{itemize}
...
{Examples: }
\begin{itemize}
\item $\{$Arthur, Bobby$\} \cap \{$Bobby, Pemberton$\}$=$\{$Bobby$\}$ \pause
\item $\{$Arthur, Bobby$\} \cap \{2, 5, 3, 8, 10\}=\emptyset$ \pause
\end{itemize}
...
\begin{itemize}
\item $\{$Arthur, Bobby$\} \cap \{$Bobby, Pemberton$\} \not=$ Bobby \pause
\end{itemize}

If two sets have an empty intersection, they are \textit{disjoint} \pause

\begin{itemize}
\item $\{$Arthur, Bobby$\}$ and $  \{2, 5, 3, 8, 10\}$ are disjoint
\end{itemize}
