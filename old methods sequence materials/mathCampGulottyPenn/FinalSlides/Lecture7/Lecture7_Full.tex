...
Mathematics calls measures of distance a norm, written $||\cdot||$. \pause

The lenght of vectors in $R^n$ can be measured using the \textbf{Euclidean} norm.\pause
...
\ \\An example. What's the Euclidean norm of $x = (1, 3, 5)$?\pause
...
$$x\cdot y =\sum_{i=1}^n x_i y_i= x_{1}*y_{1}+ x_{2}* y_{2} +...+x_{n}*y_{n}$$
\end{frame}
...
\ \\Matrices collect vectors for convenience. \pause
...
\nonumber 
\end{eqnarray}
If $\boldsymbol{A}$ has $m$ \textbf{r}ows $n$ \textbf{c}olumns (\textbf{R}oman \textbf{C}atholic; \textbf{R}um \& \textbf{C}oke) we will say that $\boldsymbol{A}$ is an $m \times n$ matrix.  \\
...
\pause
\boldsymbol{X}^{'} &  = &  \begin{pmatrix} 
\alert<3>{x_{11}} &  \alert<4>{x_{21} } & \hdots& \alert{x_{m1} } \\
\alert<3>{x_{12}} & \alert<4>{x_{22} } & \hdots &\alert{x_{m2} } \\
\vdots & \vdots & \ddot & \vdots\\
\alert<3>{x_{1n} } & \alert<4>{x_{2n}} &\hdots & \alert{x_{mn} }\\
\end{pmatrix} 
\nonumber 
\end{eqnarray}


If $\boldsymbol{X}$ is an $m \times n$ then $\boldsymbol{X}^{'} $ is $n \times m$.  \\
If $\boldsymbol{X} = \boldsymbol{X}^{'}$ then we say $\boldsymbol{X}$ is symmetric. 
...
Matrix multiplication is at the root of most econometrics, statistical modeling, but the procedure is not straightforward.\\

Suppose we have two matrices \pause  \\
$\boldsymbol{X} = \begin{pmatrix} 1 & 1 \\ 1& 2 \\ \end{pmatrix} $, $\boldsymbol{Y} = \begin{pmatrix} 1 & 2 \\ 3 & 4 \\ \end{pmatrix} $ \pause 

We will create a new matrix $\boldsymbol{A}$ by matrix multiplication: \pause 
\begin{eqnarray} 
\boldsymbol{A} & = & \boldsymbol{X} \boldsymbol{Y} \nonumber  \pause \\
				& = & \begin{pmatrix} \alert<8-9>{1} & \alert<8-9>{1} \\ \alert<10-11>{1}& \alert<10-11>{2}\\ \end{pmatrix} \begin{pmatrix} \alert<8,10>{1} & \alert<9,11>{2} \\ \alert<8,10>{3} & \alert<9,11>{4} \\ \end{pmatrix}
\nonumber  \pause \\
     					& = & \begin{pmatrix} 
...
\begin{defn} Suppose $\boldsymbol{X}$ is an $m \times n$ matrix. We write the row vectors of $\boldsymbol{X} = \begin{pmatrix} \boldsymbol{x}_{1} \\ \boldsymbol{x}_{2} \\ \vdots\\ \boldsymbol{x}_{m} \end{pmatrix} $ 
\end{defn}

$$\boldsymbol{X} = \begin{pmatrix} 4 & 1 & 2 \\1 & 2  & 3 \\ \end{pmatrix} $$

$$\boldsymbol{X} = \begin{pmatrix} \boldsymbol{x}_{1}  \\\boldsymbol{x}_{2} \end{pmatrix} $$ 
where
$$\boldsymbol{x}_{1}=(4\  1\  2)$$
$$\boldsymbol{x}_{2}=(1\  2\  3)$$
...
\begin{defn} Suppose $\boldsymbol{X}$ is an $m \times n$ matrix. We write the column vectors of $\boldsymbol{X} = \begin{pmatrix} \boldsymbol{x}_{1} & \boldsymbol{x}_{2} & \ldots& \boldsymbol{x}_{m} \end{pmatrix} $ 
\end{defn}

$$\boldsymbol{X} = \begin{pmatrix} 4 & 1 & 2 \\1 & 2  & 3 \\ \end{pmatrix} $$

$$\boldsymbol{X} = \begin{pmatrix} \boldsymbol{x}_{1}  & \boldsymbol{x}_{2} & \boldsymbol{x}_{3} \end{pmatrix} $$ 
where
$$\boldsymbol{x}_{1}= \begin{pmatrix} 4 \\1  \\ \end{pmatrix}, \ \boldsymbol{x}_{2}=\begin{pmatrix} 1 \\2\\ \end{pmatrix}, \ \boldsymbol{x}_{3}=\begin{pmatrix} 2 \\3\\ \end{pmatrix}$$
...
\begin{frame}
\begin{defn} 
 Suppose $\boldsymbol{X}$ is an $m \times n$ matrix and $\boldsymbol{Y}$ is an $n \times k$ matrix. Write the row vectors of $\boldsymbol{X} = \begin{pmatrix} \boldsymbol{x}_{1} \\ \boldsymbol{x}_{2} \\ \vdots\\ \boldsymbol{x}_{m} \end{pmatrix} $ and $\boldsymbol{Y}$ as column vector $\boldsymbol{Y} = \begin{pmatrix} \boldsymbol{y}_{1} & \boldsymbol{y}_{2} & \hdots & \boldsymbol{y}_{k} \end{pmatrix}$ .  
 
Then the $m \times k$ matrix $\boldsymbol{A} = \boldsymbol{X} \boldsymbol{Y}$ can be written as 

\begin{eqnarray}
\boldsymbol{A} & = & \begin{pmatrix} \boldsymbol{x}_{1} \cdot \boldsymbol{y}_{1} & \boldsymbol{x}_{1} \cdot  \boldsymbol{y}_{2} & \hdots & \boldsymbol{x}_{1} \cdot  \boldsymbol{y}_{k}\\
