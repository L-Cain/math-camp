...
\begin{eqnarray}
p(x) & = & \frac{e^{ - \frac{(x - \mu)^2}{2 \sigma^2}}}{\sqrt{2 \pi }  } \nonumber \\
...
If it is a max, then $f(x)-f(c)\leq0$. \pause\\
By the definition of differentiability: \pause
\begin{eqnarray}
\lim_{x \rightarrow c^{-}} \frac{f(x) - f(c) } {x - c} \equiv \lim_{x\nearrow c} \frac{f(x) - f(c ) }{x - c } & \rightarrow & f^{'}(c)\geq0 \nonumber \\\pause 
\lim_{x \rightarrow c^{+}} \frac{f(x) - f(c) } {x - c} \equiv \lim_{x \searrow c} \frac{f(x) - f(c) } {x - c }  & \rightarrow & f^{'}(c)\leq0  \nonumber \pause
\end{eqnarray}
...
\begin{frame}{Application of Rolle's Theorem}
\begin{itemize}
\item Suppose we want to prove that $x^5+4x=1$ has exactly one solution.\pause
\item Call $f(x)=x^5+4x$.  $f$ is continuous everywhere, and $f'(x)=5x^4+4$\pause
\item Suppose that $f(x)=1$ has two solutions, $d$ and $e$, $ f(d) = 1 = f(e)$,
\item Note that $f'(x)>0$ for all $x$\pause
\item The contrapositive of Rolle's Theorem states that if there is not a $c \in (a, b)$ such that $f^{'}(c) = 0$, then $f(a) \neq f(b) $.\pause
\item But $ f(d) = 1 = f(e)$, which is a contradiction.
\item There must be only one solution.
\end{itemize}
\end{frame}
