...
\end{thm} 
\pause
\begin{proof}  Without loss of generality (WLOG) select an $\epsilon$.   We need 0 to be within $\epsilon$ of $1/N_{\epsilon}$:\pause
\begin{eqnarray}
...
\begin{frame}
\frametitle{Example 2 of Limits of Sequences}
\begin{thm} $a_n=\left\{ \frac{1}{\sqrt{n}} \right\}$ converges to 0 
\end{thm} 
\pause
\begin{proof}  Without loss of generality (WLOG) select an $\epsilon>0$.   We need 0 to be within $\epsilon$ of $1/\sqrt{N_{\epsilon}}$:\pause
\begin{eqnarray}
|\frac{ 1} {\sqrt{N_{\epsilon}}} - 0 | & < &  \epsilon  \nonumber \pause \\
\frac{1}{\epsilon^2} & < & N_{\epsilon} \nonumber
\end{eqnarray}\pause
...
\begin{frame}
\frametitle{Example 3 of Limits of Sequences}

Show that 
$$\lim_{n\rightarrow \infty} -2 - \frac{3}{n^5}=-2$$
\pause
Proof: Without loss of generality (WLOG) select an $\epsilon>0$.\pause
$$|(-2- \frac{3}{n^5}) - (-2) | <  \epsilon$$\pause
$$|\frac{3}{n^5}| < \epsilon$$\pause
Solve for $N_{\epsilon}$:
$$|\frac{3}{\epsilon}|  <  N_{\epsilon}^5$$
$$\left(\frac{3}{\epsilon}\right)^{\frac{1}{5}} < N_{\epsilon}$$
For every $n>N$, the inequality $n>N_{\epsilon}>\frac{3}{\epsilon}^{\frac{1}{5}}$,
so $n>\left(\frac{3}{\epsilon}\right)^{\frac{1}{5}}$.  So all integers $n> N_{\epsilon}>\left(\frac{3}{\epsilon}\right)^{\frac{1}{5}}$ make $\frac{3}{n^5}<\epsilon$.
...
%https://www.youtube.com/watch?v=jruOXuZz7x4
\begin{frame}
...
\ \\ Applying this result to the long run value of $a$: 
$$U(a)=a+a\delta+a\delta^2+...+a\delta^{n-1} =\frac{a}{1-\delta}$$
...
\begin{frame}
\frametitle{Formal Statement of the intermediate value theorem}
...
\begin{frame}
\frametitle{Applications of intermediate value theorem: mashed potato}
\begin{itemize}

\item[] \includegraphics[width=1.5 in]{potato.jpg} 
\item A plate of mashed potato can be evenly divided by a single straight vertical knife cut.\pause\\
\textbf{Proof}: In position $K_1$ less than half the potato is at the left of the knife, in position K2 more than half is at the left. Hence (by the intermediate value theorem) there is an intermediate position where exactly half is at one side.\\
\end{itemize}
...
\begin{frame}
\frametitle{Applications of intermediate value theorem: mashed potato and beans}
\begin{itemize}

\item[]  \includegraphics[width=1.25 in]{potato.jpg}

\item A plate of mashed potato and baked beans can be evenly divided by a single straight vertical knife cut.\pause

\item[] \textbf{Proof:} Choose one particular angle and the last result shows that you can divide the potatoes by a cut $K_\alpha$ at this angle. \pause
Then (say) there will be more than half the beans on the left of the cut.  \pause
\item[2)]  Now vary the angle continuously by 180 degrees until the knife is in the same position as before, but pointing the other way. \pause
\item[3)] At each angle, make sure you bisect the potatoes.  \pause
\item[4)] You passed through an intermediate position where both beans and potatoes were divided fairly.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Call S the set of numbers $x\in [a,\ b]$ such that $f(x)\leq u$.  We want to show that the function takes on the value $f(c)=u$ at some $c \in S$.   \pause
\item We will show that this occurs at the supremum (the lowest number greater than or equal to every element of S).  \pause
...
\begin{frame}
\frametitle{Prerequisite: Least upper bound property of Real Numbers}
\begin{itemize}
\item If $S\subset\mathbb{R}$, $\sup S$ is the is the smallest real number $r$, such that $s\leq r$ for all $s\in S$.\pause
\item Example: If $S=(3,5)\cup (4, 8]$, $\sup S=8$.  \pause
\item Example: If $S=(0,1)$, $\sup S=1$.  Here $\sup S\notin S$.\pause
\item Important Result: Every nonempty subset of the real numbers with an upper bound has a supremum.\pause
\item Example: If $S=\{x: x>3\}$, $\nexists \sup S$.
...
\begin{frame}
\frametitle{Proof of intermediate value theorem step 1}
\begin{itemize}
\item[] Let $f: [a,\ b] \rightarrow \mathbb{R}$ be a continuous function.  Let $u$ be a number between  $f(a)$ and $f(b)$, where $f(a)\leq f(b)$.\pause
\item[] Consider $S=\{x \in [a,\ b]:\ f(z)\leq u\text{ for all } z\in[a, x]\}$\pause
\end{itemize}
...
\item[] Let $f: [a,\ b] \rightarrow \mathbb{R}$ be a continuous function.  Let $u$ be a number between  $f(a)$ and $f(b)$, where $f(a)\leq f(b)$.\pause
\item[] Consider $S=\{x \in [a,\ b]:\ f(z)\leq u\text{ for all } z\in[a, x]\}$\pause
\item[] $S$ is nonempty, as $a\in S$.\pause
\item[] $S$ has an upper bound, $b$. \pause
\item[] Therefore $S$ has a least upper bound, we claim it is $c$ and $f(c)=u$
\end{itemize}

\end{frame}

\begin{frame}
\frametitle{Proof of intermediate value theorem step 2}
\begin{itemize}
\item[]  Fix some $\epsilon>0$
\item By continuity, there is a $\delta>0$ such that $|x-c|<\delta$ implies $|f(x)-f(c)|<\epsilon$.\pause
\item Unpacking the absolute value bars:
\begin{align*} 
|f(x)-f(c)| &<\epsilon \text{  if   }|x-c|<\delta \pause\\
-\epsilon<f(x)-f(c)&<\epsilon\text{  if   } -\delta<x-c<\delta\\ \pause
f(x)-\epsilon< f(c)&<f(x)+\epsilon\text{  if   }  x\in (c-\delta,\ c+\delta)\\ \pause
\end{align*}
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Proof of intermediate value theorem step 2}
$$f(x)-\epsilon< f(c)<f(x)+\epsilon\text{  if   }  x\in (c-\delta,\ c+\delta)$$
\begin{itemize}
\item Because c is the supremum, there is some $d_1\in (c-\delta, c]$ that is in S, so 
\begin{align*} 
f(c)&<f(d_1)+\epsilon\pause\\
f(d_1)+\epsilon&<u+\epsilon
\end{align*}
\pause
\item There is some $d_2\in (c, c+\delta)$ where $d_2 \notin S$, 
$$f(d_2)-\epsilon< f(c)$$\pause
...
\begin{frame}
\frametitle{Proof of intermediate value theorem step 3}
$$ f(c)<f(d_1)+\epsilon<u+\epsilon$$
$$u-\epsilon<f(d_2)-\epsilon< f(c)$$\pause
so:\pause
$$u-\epsilon< f(c)<u+\epsilon$$
For all $\epsilon$.\pause

So $f(c)=u$
\end{frame}

%https://www.youtube.com/watch?v=B9GDUGzW6KI
%https://www.youtube.com/watch?v=JJLUGEookOQ&list=PLAvgI3H-gclZa-DVTMyUIAxM-X8NSikwu&index=78&t=0s

%
%http://web2.uwindsor.ca/math/hlynka/statcourses.html\

%https://www.youtube.com/watch?v=etP21xln8iQ&list=PLAvgI3H-gclZa-DVTMyUIAxM-X8NSikwu&index=1
