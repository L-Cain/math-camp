...
\begin{frame}
Often we will generalize the probability of success with a parameter $p$.\pause \\
...
\item all other values of x occur with probability 0.
\end{itemize}
\end{frame}
...
\begin{frame}{Condensed Math notation for Bernoulli PMF}
...
& = & \sum_{x} \left(x^2  -2E[X] x  + E[X]^2\right) p(x) \nonumber \\  \pause 
...
\pause 
\begin{proof} 
Define $Y = aX +b$.  Now, we know that $Var(Y) = E[(Y - E[Y])^2]$. \pause 
\begin{eqnarray}
Var(Y) & =& E[ (Y - E[Y])^2 ] \nonumber  \pause \\
& =& E[ ((aX + b) - E[a X + b]))^2 ] \nonumber  \pause \\
& =& E[ ((aX + b) - (a E[X] + b))^2 ] \nonumber  \pause \\
...
\begin{eqnarray} 
E[Y] & = & 1 \times P(Y = 1) + 0 \times P(Y = 0) \pause \nonumber \\
& = & \pi + 0 (1 - \pi) \nonumber  = \pi \pause \nonumber \\
...
\begin{itemize}
\item Number of wars in a year.\pause
\item Number of diseased villages in a district.\pause
\end{itemize}
\pause
Let $X$ be the number of times an event occurs in a unit of time or space.
\begin{itemize}
\item The probability that the event occurs in a given unit of time or space is the same for all the units. \pause
