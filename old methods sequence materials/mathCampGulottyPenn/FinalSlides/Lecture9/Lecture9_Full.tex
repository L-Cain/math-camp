...
\title[PLSC 43401] % (optional, nur bei langen Titeln)
...
\begin{frame}
\frametitle{Demonstration of Quadratic Form}
$$q(x, y)=ax^2+bxy+cy^2$$ \pause
...
\item Notice that we can rewrite it as: $q(\mathbf{x}) = x_1^2+(x_2-x_3)^2 \geq 0$
\end{itemize}
...
\item[-] Policy $\boldsymbol{x} \in \mathbb{R}^{n}$ that maximizes $U(\boldsymbol{x})$
...
\begin{defn} 
Let $\boldsymbol{x} \in \mathbb{R}^{n}$ and let $\delta >0$.  Define a \alert{neighborhood} of $\boldsymbol{x}$, $B(\boldsymbol{x}, \delta)$, as the set of points such that, 
\begin{eqnarray}
B(\boldsymbol{x}, \delta) & = & \{ \boldsymbol{y} \in \mathbb{R}^{n} : ||\boldsymbol{x} - \boldsymbol{y}||< \delta \}  \nonumber 
\end{eqnarray}
\end{defn}
...
\begin{defn} 
Suppose $f:X \rightarrow \mathbb{R}^{n}$ with $X \subset \mathbb{R}^{1}$ is a differentiable function.  Define the gradient vector of $f$ at $\boldsymbol{x}_{0}$, $\nabla f(\boldsymbol{x}_{0})$ as, 
\begin{eqnarray}
...
\begin{thm} 
Two Dimensional Second Derivative Test.  Suppose $f:X \rightarrow \mathbb{R}$ with $X \subset \mathbb{R}^{2}$ and $f$ twice differentiable.  Write the Hessian of $f$ at a critical value $\boldsymbol{a}$, 
\begin{eqnarray}
...
\includegraphics[width= 4 in]{3dplot.pdf}
...
Suppose legislators are considering legislation $\boldsymbol{x} \in \mathbb{R}^{2}$.  And suppose legislator $i$ has utility function $U_{i}: \mathbb{R}^{2} \rightarrow \mathbb{R}$, \pause
\begin{eqnarray}
