...
\begin{itemize}
\item We use the concept of convergence to describe limits, derivatives and integrals in the first half of the course.
\item Today we will discuss different kinds of convergence for random variables.
\begin{itemize}
\item Convergence in distribution.
\item Convergence in probability.
\item Convergence almost surely.
\item Convergence surely or pointwise.
\end{itemize}
\end{itemize}
...
\begin{frame}
\frametitle{Convergence Pointwise} 
...
\begin{frame}
\frametitle{Convergence of sequence of Random Variables}
...
Question: What can we say about $\left\{\widehat{\theta}_{i}\right\}_{i=1}^{n}$ as $n\rightarrow \infty$?  \pause 
\begin{itemize}
\item[-] Does $\widehat{\theta}_{n}(\omega)$ get arbitrarily close to $\theta(\omega)$ for all $\omega$? \pause
\item[-] Does $\widehat{\theta}_{n}(\omega)$ get arbitrarily close to $\theta(\omega)$ for all $\omega$ that have positive probability? \pause
\item[-] Does the probability that $\widehat{\theta}_{n}$ differs from $\theta$ go to 0?\pause 
\item[-] Does the sampling distribution of $\widehat{\theta}_{n}$ converge to the sampling distribution of $\theta$ as $n \rightarrow \infty$ ? 
\end{itemize}
...
\pause 
\begin{defn} 
We will say the sequence $\widehat{\theta}_{n}$ converges \textbf{in probability} to $\theta$  if, 
\begin{eqnarray}
...
\begin{itemize}
\item The probability of an `unusual' outcome becomes smaller and smaller as the sequence progresses.
\item[-] $\epsilon$ is a tolerance parameter: how much error around $\theta$? \pause 
\item[-] In the limit, convergence in probability implies sampling distribution collapses on a spike at $\theta$. \pause 
\end{itemize}

\end{frame}

\begin{frame}
\frametitle{Example of Convergence in Probability}
\begin{itemize}
\item Toss a fair coin $n$ times.  Let $R_n$ be the `longest run of heads".  
\item So if $n=15$ and our experiment gets, HHTTHHHTHTHTHHH, $R_15=3$.
\item As $n\rightarrow \infty$, $\frac{R_n}{log_2 n}\rightarrow 1$
\item Plugging in 20 for $R_n$, we find that $n=2^{20}=1048576$.
\end{itemize}
...
\begin{frame}
\frametitle{Example of Convergence in Probability (Consistency of $S^2$}
\begin{itemize}
\item An estimator is called consistent if it converges in probability to the quantity being estimated.
\item $S^2$ is the sample analogue of $\sigma^2$.
$$S^2_n=\frac{1}{n-1}\sum(X_i-\bar{X}_n)^2$$\pause
\item Suppose we have iid random variables $E(X_i)=\mu$ and $Var(X_i)=\sigma^2$
\item Chebychev tells us that 
$$P(|S^2-\sigma^2|\geq \epsilon)\leq \frac{E(S^2_n-\sigma^2)^2}{\epsilon^2}=\frac{VarS^2_n}{\epsilon^2}$$.
\item So $S^2$ converges in probability to $\sigma^2$ so long as $VarS^2_n\rightarrow 0$ as $n\rightarrow \infty$
...
\begin{itemize}
\invisible<1-2>{\item[-] This is stronger than convergence in probability, it says that sequence converges to $\theta$ (almost everywhere) ) } \pause 
...
\begin{frame}
\frametitle{Example Almost Sure} 
\begin{itemize}
\item Consider an animal of some short-lived species. We record the amount of food that this animal consumes per day. This sequence of numbers will be unpredictable, but we may be quite certain that one day the number will become zero, and will stay zero forever after.
\end{itemize}
...
\invisible<1-4>{\item[-] Says that cdfs are nearly the same, says nothing about convergence of underlying RV } \pause 
...
\begin{frame}
\frametitle{Example of Convergence in Distribution}
\begin{itemize}
\item Suppose a new dice factory has just been built. The first few dice come out quite biased, due to imperfections in the production process. The outcome from tossing any of them will follow a distribution markedly different from the desired uniform distribution. 

\item As the factory is improved, the dice become less and less loaded, and the outcomes from tossing a newly produced die will follow the uniform distribution more and more closely.
\end{itemize}
\end{frame}
...
\frametitle{Example of Convergence in Distribution}
\begin{itemize}
\item Suppose that X is a continuous random variable with range $\mathbb{X}_n\equiv (0,n]$ for $n>0$ and cdf:
$$F_{X_n}(x)=1-(1-\frac{x}{n})^n,\ \quad 0<x\leq n$$
\item As $n\rightarrow \infty$, 
$$F_{X_n}(x)\rightarrow 1-e^{-x}$$
\item So $X_n \xrightarrow[]{d} X$, $X\sim$Exponential(1)
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{Example of Convergence in Distribution}
\includegraphics[width=5 in]{approxexp.pdf}
\end{frame}
...
\item Does the estimator converge in probability to the estimated quantity?\pause \alert{Consistency}
\end{itemize}
\end{frame}
