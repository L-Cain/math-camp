...
\begin{theorem}
Proposition: If $F$ has a continuous derivative, then $$\int_a^b f(x)=F(b)-F(a)$$
\end{theorem} \pause
...
\begin{frame}
\frametitle{Integration Examples (Fearon 1998)}

\begin{itemize}
\item  The per period benefits of defecting are $a$, the cost of punishment is $c$, which occurs at time $\Delta$.  The payoff for cooperation is $\frac{z}{r}$. We continuously discount at rate $r$.
\item The maximum benefit of a state for defecting from an agreement is:
$$\int_0^\Delta a e^{-rs} ds - \int_\Delta^{\infty} c  e^{-rs}ds$$\pause
$$=\frac{1}{r}[a[-e^{-r*\Delta}+e^{-r*0}]]-\frac{1}{r}[ce^{-r*\infty} - -ce^{-r*\Delta}]$$
$$=\frac{1}{r}[a[-e^{-r*\Delta}+1]-ce^{-r*\Delta}]$$\pause
\item Cooperate if:
$$\frac{z}{r}\geq \frac{1}{r}[a[1-e^{-r*\Delta}]-ce^{-r*\Delta}]$$
\end{itemize}
...
\begin{frame}
\frametitle{Integration Examples (Expectation)}

\begin{itemize}
\item  If X is a continuous random variable, it has a distribution f(x) and satisfies the following:
...
\item The expected value of the random variable is $E[X]=\int_{-\infty}^{\infty}x f(x) dx$

\item The uniform distribution over $[a, b]$ has pdf $f(x)={1\over{b-a}}$ for $x\in [a,b]$ and $f(x)=0$ otherwise
