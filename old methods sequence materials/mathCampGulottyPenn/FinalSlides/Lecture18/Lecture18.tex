...
\begin{frame}{Using t-tests to evaluate the difference in Means}
We are often interested in hypotheses about averages across groups:\pause
\begin{itemize}
\item Is the average level of corruption greater in Asia than in Latin America?\pause
\item Is the salary higher for men or women?\pause
\end{itemize}
The answer depends on the nature of the data.\pause
\begin{itemize}
\item Are the samples independent or are they paired off?\pause
\item Do the samples have equal or unequal variances?\pause
\item Do the samples have equal or unequal samples sizes?
\end{itemize}
...
\begin{frame}{Equal sample size, equal variance}
Often in clinical or lab trials, we might be sure that two samples are from the same distribution, but we typically do not know the distribution's variance\pause
\begin{itemize}

\item Suppose that $\bar{X}_1$, $\bar{X}_2$ are sample means and $s_1$, $s_2$ are the sample standard deviations.\pause
\item $H_0:\ \mu_1-\mu_2=0$,  $H_1:\ |\mu_1-\mu_2|>0$\pause
\item Our \emph{estimator} is $\bar{X}_1-\bar{X}_2$\pause
\item Our best estimate of the variance is the weighted sum of the sample variances\pause
$$Var(\bar{X}_1-\bar{X}_2)=\frac{s_1^2}{N_1}+\frac{s_2^2}{N_2}$$\pause
\item Our \emph{test statistic} is:\pause
$$T=\frac{(\bar{X}_1-\bar{X}_2)-(\mu_1-\mu_2)}{\sqrt{\frac{s_1^2}{N_1}+\frac{s_2^2}{N_2}}}$$
\item T is distributed t with $N_1+N_2-2$ degrees of freedom.

\end{itemize}
\end{frame}

\begin{frame}{Equal sample sizes: Democracy and Development}
\begin{table}[htdp]
...
\hline
$N_1 = 6 $&$N_2 = 6$\\
$\bar{X}_1 = 5200$ &$\bar{X}_2 = 4500$\\
$s_1^2 = 400000$& $s_2^2 = 400000$\\
\hline
\end{tabular}
\end{center}
\label{default}
$qt(.975, 12-2)=2.22$\pause
$$T=\frac{(\bar{X}_1-\bar{X}_2)-(\mu_1-\mu_2)}{\sqrt{\frac{s_1^2}{N_1}+\frac{s_2^2}{N_2}}}$$\pause
$$T=\frac{(5200-4500)}{\sqrt{\frac{400000}{6}+\frac{400000}{6}}}=1.91$$\pause
$1-pt(1.917, 12-2)=0.042$
\end{table}
\end{frame}
...
\begin{itemize}
\item When you have two or more categorical variables based on a single population, you can use a chi-square test to test for independence of those variables.\pause

\item Example: Say you classify all the countries in the world into levels of democracy (low,medium, high), and levels of economic development (low, medium, high).\pause
\end{itemize}
\begin{table}[htdp]
\caption{Development Levels and Democracy}
\begin{center}
\begin{tabular}{l|ccc|c}
\hline
&Lowdem &Mediumdem &Highdem &total\\
\hline
Lowdev& 40 &30& 26 &96\\
Mediumdev& 5& 40& 5& 50\\
Highdev &5& 20& 25& 50\\
...
\begin{frame}{Testing Independence with Tables}
\begin{itemize}
\item First we recharacterize margins of data as proportions.\pause
\item Then we use the marginal probabilities to generate expected proportions if they were independent.\pause
\item Multiply each expected proportion by the sample size to get the expected counts.\pause
\item Calculate a test statistic:
$$T=\sum\frac{(\text{observed}-\text{expected})^2}{\text{expected}}$$\pause
\item $T\sim \chi^2_{(n_{rows}-1)\times(ncols-1)}$
\end{itemize}
\end{frame}



\begin{frame}{Degrees of Freedom in table based tests}
\begin{itemize}
\item In a proportion table, we can fill in the last row or column by process of elimination.\pause
\item  The degree of freedom is minimum number of independent data that can specify a system.\pause
\item In the previous example, we have (3-1)*(3-1)= 4 degrees of freedom, and a chi-square test statistic of $\chi^2_4(.95)=9.48$
\item The number of observations minus the number of necessary relations among these observations.
\end{itemize}
\end{frame}

\begin{frame}{Calculate Marginal Proportions}
\begin{table}[htdp]
\caption{Development Levels and Democracy}
\begin{center}
\begin{tabular}{l|ccc|c}
...
Highdev && && 50/196\\
\hline 
total &50/196& 90/196& 56/196& 196/196
\end{tabular}
\end{center}
\end{table}
\end{frame}

\begin{frame}{Calculate Expected Proportions under Indepencence}
\begin{table}[htdp]
\caption{Development Levels and Democracy}
\begin{center}
\begin{tabular}{l|ccc|c}
...
&Lowdem &Mediumdem &Highdem &total\\
\hline
Lowdev& 96/196*50/196&96/196*90/196& 96/196*56/196&96/196\\
Meddev&50/196*50/196&50/196*90/196& 50/196*56/196& 50/196\\
Highdev &50/196*50/196&50/196*90/196& 50/196*56/196& 50/196\\
\hline 
total &50/196& 90/196& 56/196& 196/196
\end{tabular}
\end{center}
...
\begin{frame}{Calculate Expected Counts under Independence}
\begin{table}[htdp]
\caption{Development Levels and Democracy}
\begin{center}
\begin{tabular}{l|ccc|c}
...
&Lowdem &Mediumdem &Highdem &total\\
\hline
Lowdev& 24.9&44& 27.4&96.3\\
Meddev&12.75&23&14.3&50.05\\
Highdev &12.75&23&14.3&50.05\\
\hline 
total &49.9&90&56&
\end{tabular}
\end{center}
...
\end{center}
\end{table}\pause
$$T=\frac{(40-24.9)^2}{24.9}+\frac{(30-44)^2}{44}+\frac{(26-27.4)^2}{27.4}$$\pause
$$+\frac{(5-12.75)^2}{12.75}+\frac{(40-23)^2}{23}+\frac{(5-14.3)^2}{14.3}$$\pause
$$+\frac{(5-12.75)^2}{12.75}+\frac{(20-23)^2}{23}+\frac{(25-14.3)^2}{14.3}=50.12>\chi^2_4(.95)$$

\end{frame}

\begin{frame}{Bootstrapping}
\begin{itemize}
\item So far we have used the central limit theorem or statistical reasoning to posit distributions.
\item Is our data really from the distribution we thought?
\item An alternative is to use bootstrapping.
\end{itemize}
\end{frame}
...
\column{0.5\textwidth}
\only<1>{\scalebox{0.25}{\includegraphics{Figure1.png}}}
\only<2>{\scalebox{0.25}{\includegraphics{Figure2.png}}}
\only<3>{\scalebox{0.25}{\includegraphics{Figure3.png}}}
\only<4>{\scalebox{0.25}{\includegraphics{Figure4.png}}}
