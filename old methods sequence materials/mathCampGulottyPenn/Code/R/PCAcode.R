...
model.matrix(~type, data=Prestige)
...
eigenvectors <- UNeigsbase$vectors

plot(1:51, eigenvalues, type="o", log="y", main="Magnitudes")
...
text(votematcov_score[,1],votematcov_score[,2], 
     labels=rownames(votematcov_score))
...
prdf3<-princomp(df3[,c(-1,-2)])

summary(prdf3)

biplot(princomp(df3[sample(1:nrow(df3), 250),c(-1,-2)]))
