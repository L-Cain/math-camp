...
\item  Why is it that: 
\begin{enumerate}
\item[a)] If $A \subset B$ then $P(B|A) = 1$, because If A happens then B is sure to have happened.  $P(A\cap B)= P(A)$ so $P(B|A)=P(A)/P(A)=1 $
\item[b)] If $A \subset B$ then $P(A|B) = \frac{P(A)}{P(B)}$.  $P(A\cap B)= P(A)$ so $P(A|B)=P(A)/P(B) $
\end{enumerate}
...
Call A the event where there is at least one six.  Call B the event where there are at least two six.  $B \subset A$, so $P(B|A)=\frac{P(B)}{P(A)}$  So P(at least two six| at least one six)=(3*1/6*1/6*5/6+1/6*1/6*1/6)/(1-5/6*5/6*5/6)

\item TRUE or FALSE, if false, explain in one sentence.
\begin{enumerate}
\item[a)] $\bar{X}=\mu$  FALSE $E(\bar{X})=\mu$
\item[b)] If $X$ and $Y$ are independent, $Var(X-Y)=Var(X)+Var(Y)$  TRUE
\item[c)] $\bar{X}\sim N(0,1)$ FALSE $\bar{X}\sim N(\mu, \sigma^2/n)$
\item[d)] $Var(\mu)=0$ TRUE
\item[e)] $Var(\bar{X})=\frac{\sigma^2}{n}$ TRUE
\end{enumerate}
...
\begin{enumerate}[1)]
\item If both teams are equally likely to win, what is the probability that the series lasts to exactly 4 games? 3/8
\item What is the probability that the series lasts less than 5 games? 1-3/8
\item What is the expected number of games played?  1/4*3+3/8*4+3/8*5=4.125
\end{enumerate}
...
\end{cases}$$
\begin{enumerate}[1)]
\item  Verify that f(x) is a pdf.  Draw f(x). It is.
\item What is the probability that $X > .7$? $\int_{.7}^1 4-4x dx= 4x-2x^2|_{.7}^1= 2-1.82=.18$
\item Calculate the median of X.  .5
\end{enumerate}
...
\begin{enumerate}[1)]
\item Suppose 25\% of the students have used illegal drugs. What is the probability of a `no'?  A no arises if the coin lands heads and they didn't use drugs.  so .5*.75= .375
\item  Now suppose that you conduct the survey and find that 35\% of the students give a `no' answer. What would be your estimate of the percent of students who have used illegal drugs? 70\%
\end{enumerate}
\pagebreak
...
\begin{enumerate}[a)]
\item What is E[X]? .5*(1-4)+.2*(4-4)+.3*(5)=0
\item What is Var(X)? 
\end{enumerate}

\item In class we examined a random variable $Z\sim N(0,1)$ 
\begin{enumerate}[a)]
\item What is E$[Z^2]$? 1
\item What is E$[Z^3]$? 0 (Take the derivative of the MGF)
\end{enumerate}
...
\end{enumerate}
\begin{enumerate}
\item[a)]  Which of these bets would you take, \textbf{A} or \textbf{B}?  Why? I prefer B  because of Jensen's inequality.

\item[b)] What if the equation were  $\sqrt{x}?$ $log(x)?$  Then I would prefer these A.
\end{enumerate}
\end{enumerate}
