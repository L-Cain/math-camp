...
\usepackage{color}
\usepackage{endnotes}
\usepackage{afterpage} % package for leaving blank page
\newcommand\blankpage{%
	\null
	\thispagestyle{empty}%
	\addtocounter{page}{-1}%
...
\textbf{Date:} Thursday, December 12, 2019

\textbf{PLSC 43401: Mathematical Foundations of Political Methodology}

\textbf{Final Exam}

\begin{enumerate}
	
	\item This exam has 6 sections.
	
	\item There are a total of 100 possible points (excluding bonus question).
	
	\item Answer as many questions as you can. You do not need to answer the questions in order. Try to answer the later parts of a question even if you have difficulty with earlier parts.
	
	\item Please write your answers in the space provided in the exam.
	
	\item Please clearly label your final answers where appropriate.
	
	\item Do not take this exam with you.
	
	\item If you have any clarification questions on the exam problems, feel free to ask.
...
\begin{enumerate}[1)]
\item If $P(A\cap B)=P(A)P(B)$, which of the following statements are true? And explain why the statement is true/false briefly.
\begin{enumerate}
\item[a)] $P(A|B)=P(B)$ 
%\item FALSE
\vfill
...
\item TRUE or FALSE. Also, explain why it is true/false briefly.
\begin{enumerate}
...
\begin{enumerate}[1)]
\item What is k? [Hint: you can answer the second question first.] %sqrt(2)
\vfill
...
\item Draw the conditional probability density function, $f_X(X|X>1)$.
\vfill
...
\item Suppose you are interested in the expectation of household income $X_{i}$ (in dollars) of a population, based on $n$ i.i.d samples $X_{1}, ..., X_{n}$ from the population. You use the sample mean $M_{n} = \frac{X_{1} + ... + X_{n}}{n}$ to estimate $\mathbb{E}[X_{i}]$. In addition, you have a rough guess that $\sqrt{\var(X_{i})}$ is \$100. 

\begin{enumerate}[a)]
	
	\item Give the formula for $\var(M_{n})$.
	\vfill
	
	\item How large should $n$ be so that the standard deviation of $M_{n}$ is at most 1\$?
	%$\sqrt{\var(M_{n})} = \frac{\sqrt{\var(X_{i})}}{\sqrt{n}} = \frac{10}{\sqrt{n}} \leq 1$, which implies $n \geq 100$.
	\vfill
...
\item Suppose you are interested in a random variable $X$ with expectation $\mathbb{E}[X]=55$ and population standard deviation $s=3$. You draw a sample from the population and the sample size is 50 (i.e., $n=50$). From the sample, you compute the sample mean: $\bar{X} = \frac{X_{1} + ... + X_{50}}{50}$.
\begin{enumerate}
\item Using Chebyshev, what is the probability that the absolute value of the difference between sample mean and $\mathbb{E}[X]$ is greater than six?
\vfill
...
\item Before you use the Central Limit Theorem to construct a confidence interval, please state the Lindberg-Levy CLT.
%\textbf{Solution}:
%	\begin{theorem}
